...
__global__ void
ant_solution_coarse_kernel(float *distances_matrix, float *pheromones_matrix,
                           int *prev_visited_matrix, int *ant_matrix,
  ...
  for (int coarse = 0; coarse < COARSENING_FACTOR; coarse++) {
    // printf("idx %d covers %d\n", idx, (idx * COARSENING_FACTOR) + coarse);
    int coarse_idx = idx * COARSENING_FACTOR + coarse;

    if (coarse_idx >= NUM_ANTS)
      return;

    for (int node = 0; node < NUM_NODES; node++) {
      for (int desire_node = 0; desire_node < NUM_NODES; desire_node++) {
        int current_node =
            ant_matrix[coarse_idx * SIZE_ANT_DATA + CURRENT_NODE_COL];
        bool desire_node_visited = false;
        for (int i = 0; i < NUM_NODES; i++) {
          if (desire_node == prev_visited_matrix[coarse_idx * NUM_NODES + i]) {
            desire_node_visited = true;
            break;
          }
        }
        if ((desire_node != current_node) && !(desire_node_visited)) {
          int _coarse_idx = current_node * NUM_NODES + desire_node;
          float desire = (pheromones_matrix[_coarse_idx]) *
                         (1 / (distances_matrix[_coarse_idx]));
          desires_matrix[coarse_idx * NUM_NODES + desire_node] = desire;
        } else {
          desires_matrix[coarse_idx * NUM_NODES + desire_node] = 0;
        }
      }
      ...
      if (node != NUM_NODES - 1) {
        ...
        for (int desires_node_offset = 0; desires_node_offset < NUM_NODES;
             desires_node_offset++) {
          desires_row_sum +=
              desires_matrix[coarse_idx * NUM_NODES + desires_node_offset];
        }
        ...
        for (int probability_node = 0; probability_node < NUM_NODES;
             probability_node++) {
          probability_matrix[coarse_idx * NUM_NODES + probability_node] =
              desires_matrix[coarse_idx * NUM_NODES + probability_node] /
              desires_row_sum;
        }

        // Generate random target
        curandState state;
        curand_init(clock64(), coarse_idx, 0, &state);
        float random_selection = curand_uniform(&state);
        ...
        for (int outcome = 0; outcome < NUM_NODES; outcome++) {
          probability_sum +=
              probability_matrix[coarse_idx * NUM_NODES + outcome];
          if (random_selection < probability_sum) {
            target_node = outcome;
            break;
          }
        ...
        for (int i = 0; i < NUM_NODES; i++) {
          if (ant_matrix[coarse_idx * NUM_NODES + CURRENT_NODE_COL] !=
              prev_visited_matrix[coarse_idx * NUM_NODES + i]) {
            current_node_visited = true;
          }
        }
        ...
        if (!current_node_visited) {
          for (int i = 0; i < NUM_NODES; i++) {
            if (prev_visited_matrix[coarse_idx * NUM_NODES + i] == -1) {
              prev_visited_matrix[coarse_idx * NUM_NODES + i] =
                  ant_matrix[coarse_idx * NUM_NODES + CURRENT_NODE_COL];
            }
          }
        }

        for (int i = 0; i < NUM_NODES; i++) {
          if (target_node == prev_visited_matrix[coarse_idx * NUM_NODES + i]) {
            target_node_visited = true;
          }
        }
        bool target_node_set = false;
        ...
        if (!target_node_visited) {
          for (int i = 0; i < NUM_NODES; i++) {
            if ((prev_visited_matrix[coarse_idx * NUM_NODES + i] == -1) &&
              ...
              prev_visited_matrix[coarse_idx * NUM_NODES + i] = target_node;
              ant_matrix[coarse_idx * NUM_NODES + CURRENT_NODE_COL] =
                  target_node;
              target_node_set = true;
            }
    ...
    int current_node, next_node;
    for (int i = 0; i < NUM_NODES - 1; i++) {
      current_node = prev_visited_matrix[coarse_idx * NUM_NODES + i];
      next_node = prev_visited_matrix[coarse_idx * NUM_NODES + i + 1];
      dist += distances_matrix[current_node * NUM_NODES + next_node];
    }

    current_node = prev_visited_matrix[coarse_idx * NUM_NODES + 0];
    next_node = prev_visited_matrix[coarse_idx * NUM_NODES + NUM_NODES - 1];
    dist += distances_matrix[current_node * NUM_NODES + next_node];

    if (dist < path_solution_matrix[coarse_idx * (NUM_NODES + 1) + 0]) {
      path_solution_matrix[coarse_idx * (NUM_NODES + 1) + 0] = dist;
      for (int i = 1; i < NUM_NODES + 1; i++) {
        path_solution_matrix[coarse_idx * (NUM_NODES + 1) + i] =
            (float)prev_visited_matrix[coarse_idx * NUM_NODES + i - 1];
      }
    }

    // Reset ant
    prev_visited_matrix[coarse_idx * NUM_NODES + 0] = coarse_idx;
    for (int i = 1; i < NUM_NODES; i++) {
      prev_visited_matrix[coarse_idx * NUM_NODES + i] = -1;
    }
    ant_matrix[coarse_idx * NUM_NODES + INDEX_COL] = coarse_idx;
    ant_matrix[coarse_idx * NUM_NODES + CURRENT_NODE_COL] = coarse_idx;
    ant_matrix[coarse_idx * NUM_NODES + STARTING_NODE_COL] = coarse_idx;
  }
}
...
void ant_solution(float *distances_matrix, float *pheromones_matrix,
  ...
  int num_threads = NUM_ANTS / COARSENING_FACTOR;
  ant_solution_coarse_kernel<<<num_threads, 1>>>(
      d_distances_matrix, d_pheromones_matrix, d_prev_visited_matrix,
      d_ant_matrix, d_desires_matrix, d_probability_matrix,
