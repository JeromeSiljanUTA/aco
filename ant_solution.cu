...
__global__ void ant_old_solution_kernel(float *distances_matrix,
                                        float *pheromones_matrix,
                                        int *prev_visited_matrix,
                                        int *ant_matrix, float *desires_matrix,
                                        float *probability_matrix,
                                        float *path_solution_matrix) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= NUM_ANTS)
  ...
  __shared__ int shared_prev_visited_matrix[NUM_ANTS * NUM_NODES];
  ...
  for (int node = 0; node < NUM_NODES; node++) {
    for (int desire_node = 0; desire_node < NUM_NODES; desire_node++) {
      ...
      bool desire_node_visited = false;
      for (int i = 0; i < NUM_NODES; i++) {
        shared_prev_visited_matrix[idx * NUM_NODES + i] =
            shared_prev_visited_matrix[idx * NUM_NODES + i];
        if (desire_node == shared_prev_visited_matrix[idx * NUM_NODES + i]) {
          desire_node_visited = true;
          break;
        }
      }
    ...
    if (node != NUM_NODES - 1) {
      ...
      if (!current_node_visited) {
        for (int i = 0; i < NUM_NODES; i++) {
          if (shared_prev_visited_matrix[idx * NUM_NODES + i] == -1) {
            shared_prev_visited_matrix[idx * NUM_NODES + i] =
                ant_matrix[idx * NUM_NODES + CURRENT_NODE_COL];
          }
        }
      ...
      for (int i = 0; i < NUM_NODES; i++) {
        if (target_node == shared_prev_visited_matrix[idx * NUM_NODES + i]) {
          target_node_visited = true;
        }
      }
      bool target_node_set = false;
      ...
      if (!target_node_visited) {
        for (int i = 0; i < NUM_NODES; i++) {
          if ((shared_prev_visited_matrix[idx * NUM_NODES + i] == -1) &&
            ...
            shared_prev_visited_matrix[idx * NUM_NODES + i] = target_node;
            ant_matrix[idx * NUM_NODES + CURRENT_NODE_COL] = target_node;
            target_node_set = true;
          }
        }
  ...
  int current_node, next_node;
  for (int i = 0; i < NUM_NODES - 1; i++) {
    current_node = shared_prev_visited_matrix[idx * NUM_NODES + i];
    next_node = shared_prev_visited_matrix[idx * NUM_NODES + i + 1];
    dist += distances_matrix[current_node * NUM_NODES + next_node];
  }

  current_node = shared_prev_visited_matrix[idx * NUM_NODES + 0];
  next_node = shared_prev_visited_matrix[idx * NUM_NODES + NUM_NODES - 1];
  dist += distances_matrix[current_node * NUM_NODES + next_node];
  ...
  if (dist < path_solution_matrix[idx * (NUM_NODES + 1) + 0]) {
    ...
    for (int i = 1; i < NUM_NODES + 1; i++) {
      path_solution_matrix[idx * (NUM_NODES + 1) + i] =
          (float)shared_prev_visited_matrix[idx * NUM_NODES + i - 1];
    }
  }

  // Reset ant
  shared_prev_visited_matrix[idx * NUM_NODES + 0] = idx;
  for (int i = 1; i < NUM_NODES; i++) {
    shared_prev_visited_matrix[idx * NUM_NODES + i] = -1;
  }
  ant_matrix[idx * NUM_NODES + INDEX_COL] = idx;
  ant_matrix[idx * NUM_NODES + CURRENT_NODE_COL] = idx;
  ant_matrix[idx * NUM_NODES + STARTING_NODE_COL] = idx;
